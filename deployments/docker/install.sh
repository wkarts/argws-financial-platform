...
if [[ "$MODE" == images ]]; then
  ...
  set_env .env ACME_IMAGE "ghcr.io/wkarts/argws-financial-acme:$version"
  set_env .env CLOUDPANEL_AGENT_IMAGE "ghcr.io/wkarts/argws-financial-cloudpanel-agent:$version"
else
  COMPOSE_FILE_PATH=compose.yaml
  set_env .env APP_PULL_POLICY build
  set_env .env BACKEND_IMAGE "argws-financial-api:$version"
  set_env .env FRONTEND_IMAGE "argws-financial-web:$version"
  set_env .env GATEWAY_IMAGE "argws-financial-gateway:$version"
  set_env .env ACME_IMAGE "argws-financial-acme:$version"
  set_env .env CLOUDPANEL_AGENT_IMAGE "argws-financial-cloudpanel-agent:$version"
fi
...
$SKIP_UP && {
  log "Configuração preparada em modo '$MODE' sem subir containers."
  exit 0
}
...
if [[ "$MODE" == images ]]; then
  log "Baixando imagens versionadas da release $version no GHCR"
  compose_cmd .env pull
  compose_cmd .env up -d --remove-orphans
else
  log "Construindo imagens localmente a partir do código-fonte"
