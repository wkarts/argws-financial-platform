...
DOMAIN=""; EMAIL=""; ENV_FILE="$HERE/stack.env"; LOCAL=false; INSECURE=false; PREPARE_ONLY=false
PORTAINER_URL="${PORTAINER_URL:-}"; PORTAINER_API_KEY="${PORTAINER_API_KEY:-}"; ENDPOINT_ID="${PORTAINER_ENDPOINT_ID:-1}"; STACK_NAME="${PORTAINER_STACK_NAME:-argws-financial-platform}"
usage(){ cat <<USAGE
...
while (($#)); do case "$1" in
  ...
  --url) PORTAINER_URL="$2"; shift 2;; --api-key) PORTAINER_API_KEY="$2"; shift 2;;
  --endpoint-id) ENDPOINT_ID="$2"; shift 2;; --stack-name) STACK_NAME="$2"; shift 2;;
  --env-file) ENV_FILE="$2"; shift 2;; --local) LOCAL=true; shift;; --prepare-only) PREPARE_ONLY=true; shift;; --insecure) INSECURE=true; shift;;
...
set_env "$ENV_FILE" BACKEND_IMAGE "ghcr.io/wkarts/argws-financial-api:$(get_env "$ENV_FILE" APP_VERSION)"
set_env "$ENV_FILE" FRONTEND_IMAGE "ghcr.io/wkarts/argws-financial-web:$(get_env "$ENV_FILE" APP_VERSION)"
set_env "$ENV_FILE" GATEWAY_IMAGE "ghcr.io/wkarts/argws-financial-gateway:$(get_env "$ENV_FILE" APP_VERSION)"
